...
\begin{exe}
  \ex\label{ex:subjunktion033}\begin{xlist}
    \ex[]{\label{ex:subjunktion034} Ich glaube, [dass dieser Nebensatz ein Verb enthält].}
    \ex[]{\label{ex:subjunktion035}{} [Während die Spielzeit läuft], zählt jedes Tor.}
    \ex[]{\label{ex:subjunktion036} Es fällt ihnen schwer [zu laufen].}
    \ex[*]{\label{ex:subjunktion037} [Obwohl kein Tor fiel].}
  \end{xlist}
\end{exe}

In (\ref{ex:subjunktion034}) ist die Definition des Nebensatzes erfüllt, weil \textit{enthält} ein \textsc{Tempus}-Merkmal hat und damit finit ist.
...
\WFiltTree{Subjunktion}{wfilt:subjunktion}{andere\\aus Filter \ref{wfilt:prep}}{Kann das Wort einen Nebensatz einleiten?}{Subjunktion}{Partikel\slash Adverb}
...
\WFiltTree{Konjunktion}{wfilt:advkopulaele}{Partikel}{Kann die Partikel zwei gleichartige Konstituenten verbinden?}{Konjunktion}{Rest}
...
\Zusammenfassung{%
  ...
  Konjunktionen und Subjunktionen bilden zwei separate Klassen, anders als die traditionelle Rede von den unterordnenden bzw.\ nebenordnenden (beiordnenden) Konjunktionen suggeriert.
}
