...
Bei \textit{säen} bezeichnet \textit{der Rasen} einen Gegenstand, der durch den Vorgang bzw.\ die Handlung erst erschaffen wird.
...
Das zugehörige Substantiv \textit{Obliqueheit} wird dementsprechend [ʔoˈbliːkha͡ɛt] ausgesprochen.}
...
\begin{figure}[!htbp]
    \begin{tikzpicture}[every text node part/.style={align=center}]
      ...
      \node (EnPlu)    at (9.5,0) {Plural\\\textit{\char`~e} (S4b)};
...
\begin{figure}[!htbp]
  ...
  \begin{forest}
    for tree={l sep=2em},
      [Genus?, s sep=7em, decide
        [Genitiv\\Singular?, s sep+=2em, decide,
          edge label={node[pos=0.6, above, sloped, font=\scriptsize]{mask\slash neut}}
          [Schwache\\Maskulina (S1), finall,
          ...
          [Nominativ\\Plural?, s sep+=0.5em, decide,
            ...
            [Gemischte Maskulina\\und Neutra (S3), finall,
              edge label={node[pos=0.7, above, sloped, font=\scriptsize]{-(e)n}}]
            [Starke Maskulina\\und Neutra (S2), finall,
              edge label={node[pos=0.7, above, sloped, font=\scriptsize]{\char`~er\slash \char`~e\slash-e}}]
          ]
        ]
        [Feminina (S4), finall,
          edge label={node[midway, above, sloped, font=\scriptsize]{fem}}]
      ]
  \end{forest}
  \caption{Entscheidungsbaum für substantivischen Flexionsklassen}
...
Pronomina bilden prototypischerweise alleine eine vollständige NP, so wie in (\ref{ex:gemeinsamkeitenundunterschiede073}) und (\ref{ex:gemeinsamkeitenundunterschiede073-2}).
...
\begin{exe}
  ...
  \end{xlist}
  \ex \label{ex:gemeinsamkeitenundunterschiede073-2}
  \begin{xlist}
...
Pronomina in Artikelfunktion und Artikel stehen immer vor dem Substantiv, mit dem sie in Kasus und Numerus kongruieren.
...
Pronomina und Artikel haben in einem syntaktischen Kontext \textit{Artikelfunktion}, wenn sie in einer NP vor dem Substantiv und allen Adjektiven stehen und mit diesen in Kasus und Numerus kongruieren.\index{Artikelfunktion}}

\Definition{Pronominalfunktion}{\label{def:pronfunktion}%
Ein Pronomen hat in einem syntaktischen Kontext \textit{Pronominalfunktion}, wenn es alleine eine vollständige NP bildet.
\index{Pronominalfunktion}
}

\WFiltTree{Artikel und Pronomen}{wfilt:artpron}{Artikel\slash\\ Pronomen}{Tritt der zu klassifizierende Stamm mit unterschiedlichen Flexionssuffixen je nach Artikel- oder Pronominalfunktion auf?}{Artikel \textbf{und} Pronomen}{\textbf{nur} Pronomen}

Mit \textit{Artikel und Pronomen} ist hier gemeint, dass es zwei lexikalische Wörter mit gleichlautendem Stamm gibt (\zB ein Pronomen \textit{kein} und einen Artikel \textit{kein}), die aber nicht in allen Formen die gleichen Flexionsendungen haben.
Mit \textit{nur Pronomen} ist gemeint, dass der Stamm (\zB \textit{dies}) nur zu einem lexikalischen Wort gehört, und zwar zu einem Pronomen.
Sowohl in Artikelfunktion als auch in Pronominalfunktion hat dieses Pronomen in allen Formen dieselben Endungen.
...
Alle anderen Wörter mit Artikel- und Pronominalflexion sind demnach ausschließlich in die Klasse der Pronomina einzuordnen.
Reine Artikelstämme, die nur zu einem Artikel (und nicht auch zu einem Pronomen) gehören, gibt es nicht.
...
Die Notwendigkeit einer Unterscheidung zwischen Genitiv und Dativ zum Beispiel hängt im Wesentlichen nur noch an diesen Pronominalendungen sowie dem Genitiv Singular der nicht-femininen Substantive auf \textit{-es} und Dativ Plural der Substantive auf \textit{-en}.
...
Zugrundeliegend wäre dann /da-əs/ und /diː-ə/, was beides phonotaktisch im Deutschen unmöglich ist und unter Ausfall des schwächsten Vokals /ə/ als [das] und [diː] ausgesprochen werden könnte.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{lllll}
    ...
    \lspbottomrule
  \end{tabular}
  \caption[Flexion des Pronomens \textit{der}]{Flexion des Pronomens \textit{der}}
...
Mir möchte das nicht akzeptabel klingen, aber ich überlasse das Urteil gerne anderen.}
...
Sie können also den Platz einnehmen, den sonst Adverben (oder andere Angaben des Verbs) einnehmen, s.\ (\ref{ex:klassifikation099}).
...
\begin{figure}[!htbp]
  \centering
  \begin{tikzpicture}[every text node part/.style={align=center}]
    \node [decide] (FlexAVoran) at (3,6) {Geht ein Artikelwort\\mit Flexionsendung voraus?};

    \node [finall] (SchwaFlexi) at (0,3) {\whyte{struktureller Singular: \textit{-e}}\\\whyte{Rest: \textit{-en}}};
    \node [finall] (pronomAffi) at (6,3) {\whyte{pronominale}\\\whyte{Affixe}};
...
\begin{Vertiefung}{Sogenannte Zahlwörter}

\label{vert:zahlwörter}

\index{Zahlwort}

\noindent Man liest und hört häufiger etwas vom \textit{Zahlwort}, als handle es sich um eine eigene Kategorie im Sinne einer Wortklasse.
Semantisch gesehen ist Zahlwörtern wie \textit{zwei} oder \textit{drei} in (\ref{ex:zahlwoerter001}) durchaus eigen, dass sie eine exakte Quantifizierung bewirken.
In den Beispielen sind die ganzen Nominalphrasen eingeklammert.

\begin{exe}
  \ex\label{ex:zahlwoerter001} 
  \begin{xlist}
    \ex\label{ex:zahlwoerter002} Beim Synchronspringen springen [zwei Wasserspringerinnen] gleichzeitig.
    \ex\label{ex:zahlwoerter003} Bis nächste Woche sollen [drei Kapitel] gelesen werden.
    \ex\label{ex:zahlwoerter0033} Bis nächste Woche sollen [die vier Kapitel] über Syntax gelesen werden.
  \end{xlist}
\end{exe}

Anders als ein nicht genau bestimmter Plural, der von einem Artikelwort wie \textit{einige} oder durch den Plural ohne Artikelwort wie in (\ref{ex:zahlwoerter004}) markiert wird, wird die genaue Anzahl der Objekte spezifiziert.

\begin{exe}
  \ex\label{ex:zahlwoerter004} 
  \begin{xlist}
    \ex{\label{ex:zahlwoerter005} [Einige Wasserspringerinnen] haben Chancen auf eine Medaille.}
    \ex{\label{ex:zahlwoerter006} [Kenntnisse] in Grammatik sind im Deutsch-Lehramt unabdinglich.}
  \end{xlist}
\end{exe}

Semantisch würde man alle Wörter, die (numerisch oder unspezifisch) eine Menge spezifizieren, als \textit{Quantoren} bezeichnen.
Wörter wie \textit{zwei} und \textit{drei} sind {kardinale Quantoren}.

Davon zu unterscheiden sind \textit{Ordinalwörter} wie \textit{zweit} oder \textit{dritt} in (\ref{ex:zahlwoerter007}), auch wenn sie oft in einem Zug mit den kardinalen Quantoren abgehandelt werden.
Sie quantifizieren nicht, sondern benennen nur ein Objekt in einer abzählbaren Reihe.

\begin{exe}
  \ex\label{ex:zahlwoerter007} 
  \begin{xlist}
    \ex{\label{ex:zahlwoerter008} [Der zweite Versuch] erhielt eine hohe Wertung.}
    \ex{\label{ex:zahlwoerter009} [Die dritte Vorlesung] war bisher die beste.}
  \end{xlist}
\end{exe}

Bezüglich der grammatischen Einordnung dieser Wörter kann die grobe semantische Bezeichnung als \textit{Zahlwort} allerdings Verwirrung stiften.
Bei den kardinalen Quantoren sind zwei Fälle zu unterscheiden, nämlich solche wie \textit{zwei Wasserspringerinnen} in (\ref{ex:zahlwoerter002}) und solche wie \textit{die vier Kapitel} in (\ref{ex:zahlwoerter0033}).
Im ersten Fall (\textit{zwei Wasserspringerinnen}) steht das Zahlwort \textit{unflektiert} vor dem Substantiv.
Wir können es also weder als Adjektiv noch als Artikel oder Pronomen einordnen, denn alle diese Nomina haben wir über ihre Flexionseigenschaften definiert.
Zudem steht in Fällen wie \textit{die vier Kapitel} ein Artikelwort noch vor dem Zahlwort.
Wenn wir nun die zwei Beispielpaare in (\ref{ex:zahlwoerter010}) und (\ref{ex:zahlwoerter013}) in den Blick nehmen, wird die Situation schnell klar.

\begin{exe}
  \ex\label{ex:zahlwoerter010} 
  \begin{xlist}
    \ex{\label{ex:zahlwoerter011} Das Publikum sah [tolle Sprünge].}
    \ex{\label{ex:zahlwoerter012} Das Publikum sah [zwei tolle Sprünge].}
  \end{xlist}
  \ex\label{ex:zahlwoerter013} 
  \begin{xlist}
    \ex{\label{ex:zahlwoerter014} Das Publikum sah [die tollen Sprünge].}
    \ex{\label{ex:zahlwoerter015} Das Publikum sah [die zwei tollen Sprünge].}
  \end{xlist}
\end{exe}

\index{Kardinalzahlwort}

Der indefinite Plural in (\ref{ex:zahlwoerter011}) wird im Deutschen typischerweise ohne Artikel formuliert, und das Adjektiv (hier \textit{tolle}) flektiert pronominal.
Das Kardinalzahlwort in (\ref{ex:zahlwoerter012}) steht einfach unflektiert davor.
Der definite Plural wird immer durch ein definites Artikelwort (Artikel oder Pronomen) markiert, in (\ref{ex:zahlwoerter014}) durch \textit{die}.
Auch hier kann das Kardinalzahlwort unflektiert hinzutreten, steht dann aber \textit{zwischen} Artikelwort und Adjektiv.
Damit wurde nebenbei gezeigt, dass es ohne erhebliche Zusatzannahmen nicht sehr plausibel ist, anzunehmen, dass pronominal ("`stark"') flektierte Adjektive syntaktisch auch die Position der Artikelwörter einnehmen (dazu auch Abschnitt~\ref{sec:diestrukturdernp}).
Die Kardinalia trennen gleichsam die beiden Positionen syntaktisch.
Wegen ihres besonderen morphologischen und syntaktischen Verhaltens wäre es angebracht, für die Kardinalia eine eigene Wortklasse (innerhalb der nicht-flektierenden Wörter) zu definieren.
Beachten Sie aber auch Übung~\ref{exc:nominalflexion07}.

\index{Adjektiv!ordinales Zahlwort}
Ordinalia verhalten sich vollständig anders und haben morphosyntaktisch nichts mit dem Kardinalzahlwort zu tun.
Sie verhalten sich syntaktisch wie Adjektive und flektieren nach den Regeln der Adjektivflexion.
Daher behandeln wir sie als Adjektive.
In (\ref{ex:zahlwoerter016}) finden sich einige Beispiele.

\begin{exe}
  \ex\label{ex:zahlwoerter016}
  \begin{xlist}
    \ex{\label{ex:zahlwoerter017} [Der zweite Sprung] erhielt eine hohe Wertung.}
    \ex{\label{ex:zahlwoerter018} [Ein zweiter Sprung] ist eine neue Chance.}
    \ex{\label{ex:zahlwoerter019} Die Schwierigkeit [des zweiten Sprungs] ist meist höher als die des ersten.}
    \ex{\label{ex:zahlwoerter020} [Einige zweite Sprünge] sind heute misslungen.}
  \end{xlist}
\end{exe}
...
Siglen: A10\slash JAN.00664, A10\slash JAN.00771, A10\slash JAN.00007, A10\slash JAN.00128, NUZ09\slash SEP.02362, A10\slash JUN.00218, BRZ10\slash MAI.06889, E98\slash MAI.12950, A10\slash MAI.00221, A09}
...
\Uebung[\tristar]{} \label{exc:nominalflexion07} Ordnen Sie die folgenden Beispiele angesichts der Analyse von Kardinalzahlwörtern in Vertiefung~\ref{vert:zahlwörter} ein.
Revidieren Sie ggf.\ die Klassifikation oder formulieren Sie Ausnahmen.
Überlegen Sie, welche Kardinalia die Ausnahmen noch betreffen könnten.

\begin{exe}
  \ex
  \begin{xlist}
    \ex[ ]{[Zwei Sportlerinnen] teilten sich die Goldmedaille.}
    \ex[ ]{Die Mannschaft feierte ausgiebig [zwei Sportlerinnen].}
    \ex[ ]{Der DSV dankte besonders [zwei Sportlerinnen].}
    \ex[*]{Die Erfolge [zwei Sportlerinnen] wurden besonders gewürdigt.}
    \ex[ ]{Die Erfolge [zweier Sportlerinnen] wurden besonders gewürdigt.}
  \end{xlist}
  \ex
  \begin{xlist}
    \ex[ ]{[Vier Sportlerinnen] teilten sich die Goldmedaille.}
    \ex[ ]{Die Mannschaft feierte ausgiebig [vier Sportlerinnen].}
    \ex[ ]{Der DSV dankte besonders [vier Sportlerinnen].}
    \ex[*]{Die Erfolge [vier Sportlerinnen] wurden besonders gewürdigt.}
    \ex[*]{Die Erfolge [vierer Sportlerinnen] wurden besonders gewürdigt.}
  \end{xlist}
  \ex
  \begin{xlist}
    \ex[ ]{[Hundert Sportlerinnen] teilten sich die Goldmedaille.}
    \ex[ ]{Die Mannschaft feierte ausgiebig [hundert Sportlerinnen].}
    \ex[ ]{Der DSV dankte besonders [hundert Sportlerinnen].}
    \ex[*]{Die Erfolge [hundert Sportlerinnen] wurden besonders gewürdigt.}
    \ex[ ]{Die Erfolge [hunderter Sportlerinnen] wurden besonders gewürdigt.}
  \end{xlist}
\end{exe}
