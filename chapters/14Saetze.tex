...
\section{Unabhängiger Satz und Matrixsatz}
\label{sec:unabhaengigersatzundmatrixsatz}
...
Das sind die des \textit{unabhängigen Aussagesatzes} wie in (\ref{ex:unabhaengigersatzundmatrixsatz002}) und die des \textit{Fragesatzes mit Fragepronomen} (\textit{w}"=Fragesatz) wie in (\ref{ex:unabhaengigersatzundmatrixsatz003}), die des \textit{Entscheidungsfragesatzes} (\textit{Ja\slash Nein-Frage}) wie in (\ref{ex:unabhaengigersatzundmatrixsatz004}) und die des \textit{Relativsatzes} wie in (\ref{ex:unabhaengigersatzundmatrixsatz005}) sowie des sehr ähnlichen \textit{eingebetteten \textit{w}"=Fragesatzes} wie in (\ref{ex:unabhaengigersatzundmatrixsatz006}).

\begin{exe}
  \ex\label{ex:unabhaengigersatzundmatrixsatz001}
  \begin{xlist}
    \ex{\label{ex:unabhaengigersatzundmatrixsatz002} Wahrscheinlich hat der Arzt das Bild gekauft.}
    \ex{\label{ex:unabhaengigersatzundmatrixsatz003} Was hat der Arzt gekauft?}
    \ex{\label{ex:unabhaengigersatzundmatrixsatz004} Hat der Arzt das Bild gekauft?}
    \ex{\label{ex:unabhaengigersatzundmatrixsatz005} (Das ist das Bild,) das der Arzt gekauft hat.}
    \ex{\label{ex:unabhaengigersatzundmatrixsatz006} (Ischariot fragt sich,) was der Arzt gekauft hat.}
  \end{xlist}
\end{exe}
...
Folglich ist (\ref{ex:unabhaengigersatzundmatrixsatz008}) ein unabhängiger Satz, weil \textit{ist} ein finites Verb ist, das nicht regiert wird, bzw.\ noch allgemeiner ein Verb, das von keiner anderen Einheit syntaktisch abhängt.
Außerdem sind offensichtlich alle Valenzanforderungen des Verbs und der von ihm abhängigen Einheiten erfüllt.
Hingegen kann (\ref{ex:unabhaengigersatzundmatrixsatz009}) kein unabhängiger Satz sein, weil zwar genau ein finites Verb vorkommt, dieses aber von \textit{dass} regiert wird und damit syntaktisch von \textit{dass} abhängt.
Konstruktionen wie (\ref{ex:unabhaengigersatzundmatrixsatz010}) und (\ref{ex:unabhaengigersatzundmatrixsatz011}) können zwar als Äußerungen verwendet werden, aber in dem hier vertretenen Verständnis sind sie keine Sätze.
Aus Sicht der Grammatik ist diese Auf"|fassung durchaus zielführend, weil beide Äußerungen syntaktisch ganz anders aufgebaut sind als (\ref{ex:unabhaengigersatzundmatrixsatz008}).

\begin{exe}
  \ex\label{ex:unabhaengigersatzundmatrixsatz007}
  \begin{xlist}
    \ex{\label{ex:unabhaengigersatzundmatrixsatz008} Die Post ist da.}
    \ex{\label{ex:unabhaengigersatzundmatrixsatz009} dass die Post da ist}
    \ex{\label{ex:unabhaengigersatzundmatrixsatz010} Hurra!}
    \ex{\label{ex:unabhaengigersatzundmatrixsatz011} Nieder mit dem König!}
  \end{xlist}
\end{exe}
...
In (\ref{ex:unabhaengigersatzundmatrixsatz012}) ist zur Illustration die Matrix jeweils in [~]\Sub{M} und die eingebettete Konstituente in [~]\Sub{E} eingeklammert.
Die Interpunktion wird im Sinne der Übersichtlichkeit weggelassen.

\begin{exe}
  \ex\label{ex:unabhaengigersatzundmatrixsatz012}
  \begin{xlist}
    \ex{\label{ex:unabhaengigersatzundmatrixsatz013} [Der Arzt weiß [dass Ischariot Maler ist]\Sub{E} ]\Sub{M}}
    \ex{\label{ex:unabhaengigersatzundmatrixsatz014} Der Arzt kennt [den Mann [der die Bilder gemalt hat]\Sub{E} ]\Sub{M}}
  \end{xlist}
\end{exe}

Man würde von \textit{Nebensätzen und ihren Matrixsätzen} sprechen.
Bei Relativsätzen wie in (\ref{ex:unabhaengigersatzundmatrixsatz014}) würde man von der \textit{Matrix-NP} (hier \textit{den Mann}) sprechen, weil der Relativsatz eine unmittelbare Konstituente dieser NP ist, aber nur eine mittelbare Konstituente des ganzen Satzes.
Vgl.\ auch Abschnitt~\ref{sec:relativsaetze} zu Relativsätzen.
Genauso kann man in (\ref{ex:unabhaengigersatzundmatrixsatz015}) von der \textit{Matrix-NP} für die eingebettete PP sprechen.

\begin{exe}
  \ex{\label{ex:unabhaengigersatzundmatrixsatz015} [das Pferd [mit der schönsten Mähne]\Sub{E} ]\Sub{M}}
\end{exe}
...
Eine solche Ersetzung funktioniert nicht immer unter Beibehaltung der Semantik, und der Status eines Nebensatzes als Subjektsatz ist nicht an die praktische Möglichkeit der Ersetzung durch eine NP gebunden.
Auf jeden Fall kann man immer irgendeine NP im Nominativ finden, die den Subjektsatz ersetzt, auch wenn das Ergebnis dann etwas völlig anderes bedeutet.
NPs in anderen Kasus lassen sich aber eben nicht statt des Subjektsatzes einsetzen.
